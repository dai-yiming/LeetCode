...
// Two Sum IV - Input is a BST => put root.val into Set and traverse root.left and root.right
...
// For every current node with a value of p, we check if k-p already exists in the array. If so, we can conclude that the sum kk can be formed by using the two elements from the given tree. Otherwise, we put this value pp into the setset.
